...
<Template let:args>
  {#if !open}
    <button out:send={{key: 'foo'}} in:receive={{key: 'foo'}} on:click={() => (open = true)}>click me</button>
  {/if}
  {#if open}
    <div on:click={() => (open = false)} class="container">
      <div out:send={{key: 'foo'}} in:receive={{key: 'foo'}} class="modal">
        <h1>So Satisfying</h1>
        <p>
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Sit quidem tenetur quia? Tempora libero, ipsum
          fugiat dicta dolorum ipsam ex.
        </p>
